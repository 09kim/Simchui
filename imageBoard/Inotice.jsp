...
for(int i =0; i < boardList.size(); i++){ 
	BoardBean bb = boardList.get(i);
		...
		if(countt % 3 == 0){
		%> <tr><td><img src="../upload/<%=bb.getFile() %>" width="200" height="200" alt="No img"><br>
		<a href="content.jsp?num=<%=bb.getNum()%>">제목:<%=bb.getSubject() %><br></a>작성자:<%=bb.getName() %><br>
		<%=sdf.format(bb.getDate()) %><br> 조회수:<%=bb.getReadcount()%><br></td><% } 
		else if(countt % 3 != 0){ 
		%><td><img src="../upload/<%=bb.getFile() %>" width="200" height="200" alt="No img"><br>
		<a href="content.jsp?num=<%=bb.getNum()%>">제목:<%=bb.getSubject() %><br></a>작성자:<%=bb.getName() %><br>
		<%=sdf.format(bb.getDate()) %><br> 조회수:<%=bb.getReadcount()%><br></td>
		<%
		} countt++;} %>
...
if(startPage > pageBlock){
	%><a href="Inotice.jsp?pageNum=<%=startPage-pageBlock%>">[이전]</a><%
}
...
// 다음 끝페이지번호 전체페이지번호
if(endPage < pageCount){
	%><a href="Inotice.jsp?pageNum=<%=startPage+pageBlock %>">[다음]</a><%
}
